...
with settings as (
  select '2026-06-25 00:00:00+05:30'::timestamptz as bonus_starts_at
), league_state as (
  ...
  select
    ...
    count(pr.id) filter (
      where m.kickoff_at >= s.bonus_starts_at
    )::integer as eligible_predictions
  ...
  select
    ...
    pt.predictions_made,
    pt.points
      + case when pt.display_name = 'Risvin' then floor(pt.eligible_predictions / 4)::integer else 0 end as points,
    greatest(
      pt.wrong - case when pt.display_name = 'Risvin' then floor(pt.eligible_predictions / 4)::integer else 0 end,
      0
    )::integer as wrong,
